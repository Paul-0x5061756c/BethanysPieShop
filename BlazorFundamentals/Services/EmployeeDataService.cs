...
using BlazorFundamentals.Helper;
using System.Text;
    ...
    public async Task<IEnumerable<Employee>> GetAllEmployees(bool refreshRequired = false)
    {
        if (!refreshRequired)
        {
            ...
            if (employeeExpirationExists)
            {
                ...
                if (employeeListExpiration > DateTime.Now && await _localStorageService.ContainKeyAsync(LocalStorageConstants.EmployeesListKey))//get from local storage
                {
